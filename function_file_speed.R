...
get_ks=Vectorize(function(i,j,residuals2){
  return(1-ks.test(c(residuals2[i,-i],residuals2[-i,i]),c(residuals2[j,-j],residuals2[-j,j]))$statistic)
}, vectorize.args=c("i", "j"))


get_est_blocks=function(n,residuals){
  similarity=outer(1:n, 1:n, FUN=get_ks,residuals2=residuals^2)
  S=make.affinity(similarity,n*0.2)
  D=diag(rowSums(S))
...
simulation=function(n,ng,beta,sigma_epsilon,sigma_a_vec, sigma_b_vec,sigma_z_vec,sigma_gamma_m,rou,d,niter,covariate_options,covariate_params){
  tic("steps only do once for a given X and g")
  tic("getting exch node set")
  ...
  toc()
  tic("steps to repeat fot niter=1000 times")
  for (iter in 1:niter){
    ...
    tic("get est block")
    est_blocks=get_est_blocks(n,residuals)
    toc()
    tic("get est block node set using est block")
    nodeSetEst=node.set(est_blocks,nodeSetExch6)
    toc()
    tic("get est block omega using est block node set")
    ...
    #using dyadic clustering
    tic("get dyadic omega")

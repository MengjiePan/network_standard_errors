...
simulation=function(n,ng,beta,sigma_epsilon,sigma_a_vec, sigma_b_vec,sigma_z_vec,sigma_gamma_m,rou,d,niter,covariate_options,covariate_params){
  ...
  XtXinv.1 <- chol2inv(chol(crossprod(X_m))) 
  ...
  for (iter in 1:niter){
    ...
    coefs=mod$coefs
    residuals.v=c(residuals)[-seq(1,n^2,n+1)]
    tic("get B-E estimator with block oracle")
    meat.BE.block.oracle <- meat.S(nodeSetTrue,X_m,residuals.v,K=K) 
    beta_covariance_estparams_trueblock<-sandwich.var(XtXinv.1, meat.BE.block.oracle$M)
    ...
    toc()
    tic("get B-E estimator with estimated blocks")
    meat.BE <- meat.S(nodeSetEst,X_m,residuals.v ,K=K) 
    beta_covariance_estparams_estblock <- sandwich.var(XtXinv.1, meat.BE$M)
    beta_covariance_estparams_estblock=make.positive.var(beta_covariance_estparams_estblock)$V
    toc()

    tic("get exch estimator")
    meat.E <- meat.S(nodeSetExch5,X_m, residuals.v) 
    beta_covariance_exch<- sandwich.var(XtXinv.1, meat.E$M)
    beta_covariance_exch=make.positive.var(beta_covariance_exch)$V
    toc()
    
    tic("get DC estimator")
    meat.DC <- meat.U(nodeSetExch5,X_m, residuals.v) 
    beta_covariance_DC <- sandwich.var(XtXinv.1, meat.DC$M)
    beta_covariance_DC=make.positive.var(beta_covariance_DC)$V
    toc()
    ...
    for (beta_index in 1:p){
      ...
      CI_DC[2*beta_index-1]=coefs[beta_index]-qnorm(.975)*sqrt(diag(beta_covariance_DC))[beta_index]
      CI_DC[2*beta_index]=coefs[beta_index]+qnorm(.975)*sqrt(diag(beta_covariance_DC))[beta_index]
    ...
    CI_DC_record=rbind(CI_DC_record,CI_DC)
  }
  toc()
  ans=list(X=X,ng=ng,g=g,CI_estparams_trueblock_record=CI_estparams_trueblock_record,CI_estparams_estblock_record=CI_estparams_estblock_record,CI_exch_record=CI_exch_record,CI_DC_record=CI_DC_record)
  return(ans)
}
